...
- Base URL is `https://agentsupplytrust-api.com`. A client generated from an earlier copy of this contract points elsewhere and must be regenerated.
...
- `POST /v1/demo/scan` summary and description revised.
...
- First published contract, 9 operations:
  ...
  - `POST /v1/scans` — Scan one component
  - `POST /v1/demo/scan` (no API key required)
  - `POST /v1/verdicts` — Scan and apply a policy — returns an auditable allow/warn/deny
  - `POST /v1/scans/batch` — Scan up to 100 components in one call (CI use case)
